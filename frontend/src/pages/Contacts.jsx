  ...
  async function handleImport() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/contacts/import`, {
        ...
        body: JSON.stringify({ niche, actorId, searchTerms, maxResults: Number(maxResults) }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setMsg({ type: 'ok', text: 'Importação iniciada em background. Os contatos aparecerão em instantes.' });
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h3 className="font-semibold text-white">Importar contatos via Apify</h3>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <label className="text-sm text-gray-400 mb-1 block">Apify Actor ID *</label>
            <input
              value={actorId}
              onChange={e => setActorId(e.target.value)}
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-green-500"
            />
            <p className="text-xs text-gray-600 mt-1">Actor do Google Maps: compass/crawler-google-places</p>
          </div>
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder={"restaurantes São Paulo\nrestaurantes Rio de Janeiro"}
          ...
          <div>
            <label className="text-sm text-gray-400 mb-1 block">Quantidade máxima de resultados</label>
            <input
              type="number"
              value={maxResults}
